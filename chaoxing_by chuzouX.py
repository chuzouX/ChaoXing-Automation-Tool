...
def generate_answer_section(questions_list):
    ...
    for question in questions_list:
        ...
        if question_type == 0:  # 单选题
            answer_text = map_single_choice_answer(api_answer)
            answer_params[f'answer{question_id}'] = answer_text
        if question_type == 1:  # 多选题
            answer_text = map_multiple_choice_answer(api_answer)
            answer_params[f'answer{question_id}'] = answer_text
...
def map_single_choice_answer(answer_text):
    """
    映射单选题答案
...
def map_multiple_choice_answer(answer_text):
    """
    映射多选题答案
...
def extract_questions(text_content):
    """
    从文本内容中提取题目信息，并以JSON格式返回
    ...
    for match in matches:
        question_number = match[0]  # 题号
        question_type = match[1]    # 题型
        question_content = match[2].strip()  # 题目内容
        ...
        question_data = {
            ...
            "full_question": f"{question_number}. ({question_type}) {question_content}"
        }
    ...
    if not questions:
        questions = extract_questions_alternative(text_content)
    
    return questions

def clean_question_content(content):
    """
    清理题目内容，移除可能包含的章节标题和其他无关信息
    ...
    Returns:
        str: 清理后的题目内容
    """
    # 移除章节标题信息（如一. 单选题、二. 判断题等）
    content = re.sub(r'一\.\s*单选题.*$', '', content, flags=re.DOTALL)
    content = re.sub(r'二\.\s*判断题.*$', '', content, flags=re.DOTALL)
    content = re.sub(r'三\.\s*简答题.*$', '', content, flags=re.DOTALL)
    
    # 移除数字列表（如1 2 3 4 5等）
    content = re.sub(r'\b\d+\s+\d+.*$', '', content)
    
    # 移除"提示"等无关信息
    content = re.sub(r'提示.*$', '', content, flags=re.DOTALL)
...
def extract_questions_alternative(text_content):
    """
    备用的题目提取方法，用于处理不同格式的题目
    ...
    for line in lines:
        ...
        if match:
            # 如果之前有题目在缓冲，先保存
            if current_question is not None and question_buffer:
                ...
                question_data = {
                    ...
                    "content": question_content,
                    "full_question": f"{current_question['number']}. ({current_question['type']}) {question_content}"
                }
                questions.append(question_data)
            ...
            # 尝试提取题型
            type_match = re.match(r'\(([^)]+)\)\s*(.*)', rest_of_line)
            if type_match:
                question_type = type_match.group(1)
                question_content = type_match.group(2)
            else:
                question_type = "未知题型"
                question_content = rest_of_line
            
            current_question = {
                "number": int(question_number),
                "type": question_type
            }
            question_buffer = [question_content] if question_content else []
...
if __name__ == "__main__":
    ...
    if result['status'] == 'success':
        ...
        # 提取题目
        questions = extract_questions(result['content'])
    ...
    try:
        ...
        for q_type, count in question_types.items():
            type_name = {0: "单选题", 3: "判断题", 4: "简答题"}.get(q_type, f"未知类型({q_type})")
